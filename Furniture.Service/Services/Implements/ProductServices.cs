...
=======
        ...
        if (!string.IsNullOrWhiteSpace(product.PictureUrl))
        {
            ...
            foreach (var imageUrl in imageUrls)
            {
                ...
                if (fileExists)
                {
                    bool deleted = await _storageService.DeleteFileAsync(containerName, blobName);
                }
            }
        }
        _repository.Delete(product);
        bool saveResult = await _repository.SaveChangesAsync();
        return saveResult;
    }
    public async Task<ProductDto?> GetProductByIdAsync(Guid id)
    {
    ...
    public async Task<bool> UpdateAsync(ProductDto model)
    {
        ...
        if (model.Images != null && model.Images.Count > 0)
        {
            ...
            var oldImageNames = existingProduct.PictureUrl?
                ...
                .Select(url => Path.GetFileName(new Uri(url).LocalPath))
                .ToList() ?? new List<string>();
            bool isSameImages = newImageNames.OrderBy(x => x).SequenceEqual(oldImageNames.OrderBy(x => x));
            if (isSameImages)
            {
                var updated = _mapper.Map(model, existingProduct);
                updated.PictureUrl = existingProduct.PictureUrl!;
            ...
            var uploadedImages = await _storageService.SaveFilesAsync(containerName, model.Images);
            ...
            if (!string.IsNullOrEmpty(existingProduct.PictureUrl))
            {
                ...
                foreach (var imageUrl in oldImageUrls)
                {
                    ...
                    if (fileExists)
                    {
                        await _storageService.DeleteFileAsync(containerName, oldBlobName);
                    }
                }
            }
            newPictureUrls = uploadedImages;
        }
        else
        {
            newPictureUrls = existingProduct.PictureUrl?.Split(',').ToList() ?? new List<string>();
